import { STAKE_AAVE_PROXY, TESTNET_REWARD_TOKEN_PREFIX } from "../../../helpers/deploy-ids";
...
const func: DeployFunction = async function ({
  ...
  await Bluebird.each(reserveSymbols, async (symbol) => {
    ...
    if (symbol == poolConfig.WrappedNativeTokenSymbol) {
      ...
      await deploy(`${symbol}${TESTNET_TOKEN_PREFIX}`, {
        ...
        args: [symbol, symbol, reservesConfig[symbol].reserveDecimals, faucetOwnable.address],
        ...COMMON_DEPLOY_PARAMS,
      });
    }
  });
  ...
  if (isIncentivesEnabled(poolConfig)) {
    ...
    const rewardSymbols: string[] = Object.keys(poolConfig.IncentivesConfig.rewards[network] || {});
    ...
    const aaveTokenArtifact = await deployments.get(`AAVE${TESTNET_TOKEN_PREFIX}`);

    const stakeProxy = await deployInitializableAdminUpgradeabilityProxy(STAKE_AAVE_PROXY);
